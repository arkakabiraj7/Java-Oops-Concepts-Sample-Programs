...
class MyFrame extends JFrame implements ActionListener {
	...
	public MyFrame() {
		setTitle(" Marriage Registration Form");
		setBounds(300, 90, 900, 600);
		setDefaultCloseOperation(EXIT_ON_CLOSE);
		setResizable(true);
		c = getContentPane();
		c.setLayout(null);
		title = new JLabel(" Marriage Registration Form");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == sub) {
			if (term.isSelected()) {
				...
				row.add(tmno.getText());
				row.add(data1);
				row.add((String) date.getSelectedItem() + "/" + (String) month.getSelectedItem() + "/"
						+ (String) year.getSelectedItem());
				row.add(tadd.getText());
				model.addRow(row);
				j.setBounds(30, 70, 100, 50);
				f.add(j);
				f.setSize(550, 350);
